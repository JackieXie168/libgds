...
static void sha1_process( SSHA1Context *ctx, const uint8_t data[64] )
{
...
void sha1_update( SSHA1Context *ctx, const uint8_t *input, uint32_t length )
{
    uint32_t left, fill;
...
void sha1_finish( SSHA1Context *ctx, uint8_t digest[20] )
{
    uint32_t last, padn;
    uint32_t high, low;
    uint8_t msglen[8];
                ...
                uint8_t digest[20] )
{
    ...
    memset( k_opad, 0, 64 );
    memset( tmpbuf, 0, 20 );
    memset( &ctx, 0, sizeof( SSHA1Context ) );
}
